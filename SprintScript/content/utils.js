(function () {
    ...
    function processTextField(field) {
      ...
      if (!value) return;
      const substitutions = window.SprintScript.substitutions?.getAll() || {};
      Object.entries(substitutions).forEach(([shortcut, text]) => {
        if (value.includes(shortcut)) {
          window.SprintScript.tooltip.confirmSubstitution(field, shortcut, text, "input");
        }
      });
    }
    ...
    function processContentEditable(el) {
      ...
      Object.entries(substitutions).forEach(([shortcut, text]) => {
        if (value.includes(shortcut)) {
          window.SprintScript.tooltip.confirmSubstitution(el, shortcut, text, "contenteditable");
        }
      });
    }
